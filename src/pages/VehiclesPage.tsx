...
function VehiclesPage() {
  // --- ESTADO DEL STORE ---
  const { vehicles, loading, fetchVehicles, deleteVehicle, addVehicle, updateVehicle } = useVehicleStore();
  ...
  // --- LÓGICA DE FILTRADO, ORDENADO Y PAGINACIÓN ---
  const processedVehicles = useMemo(() => {
    const filtered = vehicles.filter(vehicle => {
      ...
      // Filtros de Columna
      const columnMatch = (Object.keys(columnFilters) as FilterableColumn[]).every(key => 
  ...
  const requestSort = (key: keyof IVehicle) => {
  ...
  const openNewModal = () => { setVehicleToEdit(null); setIsFormModalVisible(true); };
  const openEditModal = (vehicle: IVehicle) => { setVehicleToEdit(vehicle); setIsFormModalVisible(true); };
  const hideFormModal = () => { setIsFormModalVisible(false); setVehicleToEdit(null); };
  const openConfirmModal = (vehicle: IVehicle) => { setVehicleToDelete(vehicle); setIsConfirmModalVisible(true); };
  const hideConfirmModal = () => { setVehicleToDelete(null); setIsConfirmModalVisible(false); };
  ...
  const handleSave = async (data: VehicleFormData) => {
    ...
    try {
      ...
      const errorMessage = (error as any)?.response?.data?.message || 'No se pudo guardar el vehículo.';
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      console.error(error);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const errorMessage = (error as any)?.response?.data?.message || 'No se pudo eliminar el vehículo.';
      toast.error(errorMessage, { id: toastId });
    }
    hideConfirmModal();
  };
  ...
  // --- Clases de Tailwind ---
  const buttonBaseClass = "inline-flex items-center justify-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:opacity-50 transition-colors shadow-sm";
  ...
  const inputBaseClass = "block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:border-blue-400 dark:focus:ring-blue-400";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-purple-50/30 p-4 md:p-8 text-gray-900 dark:text-gray-100 dark:bg-gray-900">
      ...
      <header className="max-w-8xl mx-auto mb-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">Administra y mantiene el registro de vehículos del sistema.</p>
          </div>
          <div className="flex items-center gap-3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl p-3 shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-800 dark:text-white">{totalItems}</p> {/* Mostrar total filtrado/original */}
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    <div className="flex items-center justify-between gap-1 cursor-pointer" onClick={() => requestSort('placa')}>
                      <span>Placa</span>
                      {/* Add sort indicator icons here if desired */}
                    </div>
                    <input 
                      type="text" 
                      value={columnFilters.placa} 
                      onChange={(e) => handleColumnFilterChange('placa', e.target.value)}
                      placeholder="Filtrar..." 
                      className={`${inputBaseClass} text-xs mt-1 p-1`} 
                      onClick={(e) => e.stopPropagation()} 
                    />
                  </th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    <div className="flex items-center justify-between gap-1 cursor-pointer" onClick={() => requestSort('marca')}>
                      <span>Marca</span>
                    </div>
                     <input 
                      type="text" 
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    <div className="flex items-center justify-between gap-1 cursor-pointer" onClick={() => requestSort('modelo')}>
                      <span>Modelo</span>
                    </div>
                     <input 
                      type="text" 
                      value={columnFilters.modelo} 
                      onChange={(e) => handleColumnFilterChange('modelo', e.target.value)}
                      placeholder="Filtrar..." 
                      className={`${inputBaseClass} text-xs mt-1 p-1`} 
                      onClick={(e) => e.stopPropagation()}
                    />
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {loading ? (
                  <tr><td colSpan={4} className="px-6 py-10 text-center text-gray-500 dark:text-gray-400 italic">Cargando vehículos...</td></tr>
                ) : paginatedVehicles.length > 0 ? (
                  paginatedVehicles.map((vehicle) => (
                    <tr key={vehicle.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{vehicle.placa}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">{vehicle.marca}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">{vehicle.modelo}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div className="flex justify-center gap-2">
                          <button onClick={() => openEditModal(vehicle)} className="text-blue-600 hover:text-blue-800 p-1 rounded-md hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors" title="Editar">
                            <PencilIcon />
                          </button>
                          <button onClick={() => openConfirmModal(vehicle)} className="text-red-600 hover:text-red-800 p-1 rounded-md hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors" title="Eliminar">
                            <TrashIcon />
                          </button>
                        </div>
                      </td>
                  ...
                  <tr><td colSpan={4} className="px-6 py-10 text-center text-gray-500 dark:text-gray-400 italic">
                    {vehicles.length === 0 ? 'No hay vehículos registrados.' : 'No se encontraron vehículos con los filtros aplicados.'}
                  </td></tr>
                )}
              </tbody>
            </table>
      ...
      {/* Modals */}
      <VehicleFormModal visible={isFormModalVisible} onHide={hideFormModal} vehicleToEdit={vehicleToEdit} onSave={handleSave} isSaving={isSaving} />
                    ...
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          ¿Estás seguro de eliminar el vehículo **{vehicleToDelete?.placa}**? Esta acción no se puede deshacer.
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="mt-6 flex flex-col sm:flex-row-reverse sm:gap-3">
                    <button type="button" className={dangerButtonClass + " w-full sm:w-auto"} onClick={handleDeleteConfirm}>
                      Sí, eliminar
                    </button>
                    <button type="button" className={secondaryButtonClass + " w-full sm:w-auto mt-3 sm:mt-0"} onClick={hideConfirmModal}>
                      Cancelar
                    </button>
                  </div>
                </Dialog.Panel>
